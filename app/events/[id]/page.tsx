  ...
  return (
    <>
      ...
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1.2rem 2.5rem",position:"sticky",top:0,zIndex:100,background:"rgba(13,13,31,0.95)",backdropFilter:"blur(12px)",borderBottom:"1px solid rgba(255,209,102,0.1)"}}>
        ...
        <button className="btn" onClick={() => router.push("/")} style={{padding:"0.5rem 1.2rem",border:"1px solid #2d2d5a",borderRadius:999,background:"transparent",color:"#f0f0ff",fontSize:"0.85rem"}}>← Back to Events</button>
      </nav>

      <div style={{background:"linear-gradient(135deg,#1a1a3a,#0d0d1f)",padding:"4rem 2.5rem 3rem",position:"relative",overflow:"hidden",borderBottom:`3px solid ${c}`}}>
        ...
        <div style={{maxWidth:900,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1rem",flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:"0.7rem",fontWeight:700,padding:"0.3rem 0.8rem",borderRadius:999,background:spotsLeft<=2?"rgba(255,107,107,0.15)":"rgba(255,209,102,0.1)",color:spotsLeft<=2?"#FF6B6B":"#FFD166",border:`1px solid ${spotsLeft<=2?"rgba(255,107,107,0.3)":"rgba(255,209,102,0.2)"}`}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:"1.5rem",flexWrap:"wrap",fontSize:"0.9rem",color:"#7070a0"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"2.5rem",display:"grid",gridTemplateColumns:"1fr 300px",gap:"2rem",alignItems:"start"}}>
        <div style={{animation:"fadeInUp 0.4s ease"}}>
          <div style={{background:"#13132b",borderRadius:16,padding:"1.8rem",border:"1px solid rgba(255,209,102,0.08)",marginBottom:"1.5rem"}}>
            <h2 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.4rem",letterSpacing:1,marginBottom:"1rem",color:"#FFD166"}}>About This Event</h2>
            <p style={{color:"#a0a0c0",lineHeight:1.8,fontSize:"0.95rem"}}>{event.description}</p>
          </div>
          ...
          <div style={{background:"#13132b",borderRadius:16,padding:"1.8rem",border:"1px solid rgba(255,209,102,0.08)",marginBottom:"1.5rem"}}>
            <h2 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.4rem",letterSpacing:1,marginBottom:"1rem",color:"#FFD166"}}>What to Bring</h2>
            <div style={{display:"flex",flexDirection:"column",gap:"0.6rem"}}>
              {event.whatToBring.map(item => (
                <div key={item} style={{display:"flex",alignItems:"center",gap:"0.8rem",fontSize:"0.9rem",color:"#a0a0c0"}}>
          ...
          <div style={{background:"#13132b",borderRadius:16,padding:"1.8rem",border:"1px solid rgba(255,209,102,0.08)",marginBottom:"1.5rem"}}>
            <h2 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.4rem",letterSpacing:1,marginBottom:"1rem",color:"#FFD166"}}>House Rules</h2>
            <div style={{display:"flex",flexDirection:"column",gap:"0.6rem"}}>
              {event.rules.map((rule,i) => (
                <div key={i} style={{display:"flex",alignItems:"flex-start",gap:"0.8rem",fontSize:"0.9rem",color:"#a0a0c0"}}>
          ...
          <div style={{background:"#13132b",borderRadius:16,padding:"1.8rem",border:"1px solid rgba(255,209,102,0.08)"}}>
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:"0.8rem",fontSize:"0.9rem",color:"#a0a0c0"}}>
              ...
              <div>
                ...
                <div style={{color:"#7070a0",fontSize:"0.85rem"}}>Postcode {event.postcode} · Melbourne VIC</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{position:"sticky",top:"5rem",animation:"fadeInUp 0.4s ease 0.1s both"}}>
          <div style={{background:"#13132b",borderRadius:20,border:`1px solid ${c}30`,overflow:"hidden"}}>
            <div style={{background:"linear-gradient(135deg,#1a1a3a,#0d0d1f)",padding:"1.5rem",borderBottom:"1px solid rgba(255,209,102,0.1)"}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.8rem",color:c,letterSpacing:1}}>{event.date}</div>
              <div style={{fontSize:"0.9rem",color:"#7070a0"}}>{event.time} · {event.duration}</div>
              <div style={{marginTop:"0.8rem",fontSize:"1rem",fontWeight:700,color:event.cost==="Free"?"#FFD166":"#f0f0ff"}}>{event.cost === "Free" ? "🎉 Free to join" : `💰 ${event.cost}`}</div>
            </div>
            <div style={{padding:"1.5rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:"0.8rem",marginBottom:"1rem",paddingBottom:"1rem",borderBottom:"1px solid rgba(255,209,102,0.07)"}}>
                <div style={{width:44,height:44,borderRadius:"50%",background:`linear-gradient(135deg,${c},#2D1B69)`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.2rem",color:"#0d0d1f",flexShrink:0}}>
                  {event.hostFull.split(" ").map(w=>w[0]).join("").slice(0,2)}
                </div>
                <div>
                  <div style={{fontWeight:700,fontSize:"0.9rem"}}>{event.hostFull}</div>
                  <div style={{fontSize:"0.8rem",color:"#7070a0"}}>Event Host</div>
                </div>
              </div>
              <p style={{fontSize:"0.82rem",color:"#7070a0",lineHeight:1.6,marginBottom:"1.2rem"}}>{event.hostBio}</p>
              <div style={{background:`${c}10`,border:`1px solid ${c}25`,borderRadius:12,padding:"0.8rem 1rem",marginBottom:"1.2rem",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span style={{fontSize:"0.85rem",color:"#7070a0"}}>Spots filled</span>
                <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.3rem",color:c,letterSpacing:1}}>{event.joined}/{event.total}</span>
              </div>

              {joinSent ? (
                <div style={{background:"rgba(255,209,102,0.1)",border:"1px solid rgba(255,209,102,0.2)",borderRadius:12,padding:"1rem",textAlign:"center",marginBottom:"0.8rem"}}>
                  <div style={{fontSize:"1.5rem",marginBottom:"0.3rem"}}>✅</div>
                  <div style={{fontSize:"0.9rem",color:"#FFD166",fontWeight:600}}>Request Sent!</div>
                  <div style={{fontSize:"0.8rem",color:"#7070a0",marginTop:"0.2rem"}}>The host will confirm you soon</div>
                </div>
              ) : (
                <button className="btn" onClick={() => user ? setShowJoinModal(true) : router.push("/")}
                  style={{width:"100%",padding:"0.9rem",background:c,color:"#0d0d1f",border:"none",borderRadius:12,fontWeight:700,fontSize:"1rem",marginBottom:"0.8rem"}}>
                  {user ? `${event.emoji} Request to Join` : "🔐 Log In to Join"}
                </button>
              )}

              {!user && <p style={{textAlign:"center",fontSize:"0.8rem",color:"#7070a0"}}><span style={{color:"#FFD166",cursor:"pointer"}} onClick={() => router.push("/")}>Sign up free</span> to join this event</p>}
              <div style={{marginTop:"1rem",paddingTop:"1rem",borderTop:"1px solid rgba(255,209,102,0.07)",fontSize:"0.78rem",color:"#4040a0",textAlign:"center"}}>✓ Free to request · Host confirms your spot</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {showJoinModal && (
        <div onClick={(e) => { if(e.target===e.currentTarget) setShowJoinModal(false); }}
          ...
          <div style={{background:"#1a1a3a",border:"1px solid rgba(255,209,102,0.15)",borderRadius:20,padding:"2rem",width:"90%",maxWidth:480,position:"relative",animation:"slideUp 0.3s ease"}}>
            <button onClick={() => setShowJoinModal(false)} style={{position:"absolute",top:"1rem",right:"1rem",background:"none",border:"none",color:"#7070a0",fontSize:"1.2rem",cursor:"pointer"}}>✕</button>
            <h3 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.8rem",letterSpacing:1,marginBottom:"0.3rem"}}>Request to Join</h3>
            <p style={{color:"#7070a0",fontSize:"0.88rem",marginBottom:"1.5rem"}}>Send a message to {event.hostFull}</p>
            ...
            <button className="btn" onClick={handleJoin} style={{width:"100%",padding:"0.85rem",background:c,color:"#0d0d1f",border:"none",borderRadius:10,fontWeight:700,fontSize:"1rem"}}>Send Request →</button>
          </div>
        </div>
      )}
