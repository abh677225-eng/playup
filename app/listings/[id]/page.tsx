  ...
  return (
    <>
      ...
      <div style={{background:"#1e3a5f",padding:"4rem 2.5rem 3rem",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",fontSize:"20vw",top:"-1rem",right:"-1rem",opacity:0.05,pointerEvents:"none"}}>{getEmoji(listing!.activity_type)}</div>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1rem",flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:"0.7rem",fontWeight:700,padding:"0.3rem 0.8rem",borderRadius:999,background:"rgba(132,204,22,0.1)",color:"#84CC16",border:"1px solid rgba(132,204,22,0.2)"}}>{listing!.lesson_type} lessons</span>
          ...
          <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(2rem,5vw,3.5rem)",lineHeight:1.05,marginBottom:"1rem",letterSpacing:1,color:"white"}}>{listing!.lesson_title}</h1>
          <div style={{display:"flex",alignItems:"center",gap:"1.5rem",flexWrap:"wrap",fontSize:"0.9rem",color:"#93c5fd"}}>
            ...
            <span>📍 {suburbs.slice(0,3).map((s:any)=>s.name).join(", ")}{suburbs.length>3?` +${suburbs.length-3} more`:""}</span>
            <span>⏱️ {getDuration(listing!.session_duration)}</span>
            <span>💰 ${listing!.price}/session</span>
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"2.5rem",display:"grid",gridTemplateColumns:"1fr 300px",gap:"2rem",alignItems:"start"}}>
        ...
        <div style={{animation:"fadeInUp 0.4s ease"}}>
          ...
          {/* PROVIDER BIO */}
          <div style={cardStyle}>
            <h2 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.3rem",letterSpacing:1,marginBottom:"1.2rem",color:"#1e3a5f"}}>About the Provider</h2>
            <div style={{display:"flex",gap:"1.2rem",alignItems:"flex-start",marginBottom: listing!.description ? "1rem" : 0}}>
              {listing!.photo_url ? (
                <img src={listing!.photo_url} alt={listing!.provider_name} style={{width:72,height:72,borderRadius:12,objectFit:"cover",border:"2px solid #84CC16",flexShrink:0}}/>
              ) : (
            ...
            {listing!.description && (
              <div style={{background:"#f8faff",borderRadius:10,padding:"1rem 1.2rem",border:"1px solid #dbeafe",marginTop:"0.8rem"}}>
                <p style={{color:"#475569",lineHeight:1.8,fontSize:"0.95rem",margin:0}}>{listing!.description}</p>
              </div>
            )}
          </div>
          ...
          <div style={cardStyle}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.8rem"}}>
              {[
                ...
                ["🌐","Online lessons",listing!.online_available?"Available":"Not available"],
              ].map(([icon,label,value])=>(
                <div key={label} style={{background:"#f8faff",borderRadius:10,padding:"1rem",border:"1px solid #dbeafe",display:"flex",gap:"0.8rem",alignItems:"flex-start"}}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"0.5rem"}}>
              {suburbs.map((s:any,i:number)=>(
                <span key={i} style={{display:"inline-flex",alignItems:"center",gap:"0.3rem",background:"#EAF3DE",border:"1px solid #97C459",borderRadius:999,padding:"0.35rem 0.9rem",fontSize:"0.82rem",color:"#27500A",fontWeight:600}}>
                  📍 {s.name} <span style={{fontWeight:400,color:"#4d7c0f"}}>{s.postcode}</span>
                </span>
              ))}
            </div>
            {listing!.online_available&&(
              <div style={{marginTop:"0.8rem",display:"flex",alignItems:"center",gap:"0.5rem",fontSize:"0.85rem",color:"#475569",background:"#EAF3DE",borderRadius:8,padding:"0.6rem 0.9rem",border:"1px solid #97C459"}}>
                🌐 <span>This provider also offers <strong style={{color:"#27500A"}}>online lessons</strong> — no travel required</span>
              </div>
            )}
          </div>
        ...
        <div style={{position:"sticky",top:"5rem",animation:"fadeInUp 0.4s ease 0.1s both"}}>
          <div style={{background:"white",borderRadius:20,border:"1px solid #dbeafe",overflow:"hidden",boxShadow:"0 4px 20px rgba(30,58,95,0.1)"}}>
            ...
            <div style={{padding:"1.5rem"}}>
              ...
              <div style={{fontSize:"0.75rem",color:"#94a3b8",textAlign:"center",paddingTop:"0.8rem",borderTop:"1px solid #f1f5f9",lineHeight:1.6}}>
                ✓ Free to enquire<br/>✓ No booking fees<br/>✓ In-app messaging
              </div>
            </div>
          </div>
        </div>
