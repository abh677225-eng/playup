  ...
  return (
    <>
      ...
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1.2rem 2.5rem",position:"sticky",top:0,zIndex:100,background:"rgba(255,255,255,0.95)",backdropFilter:"blur(12px)",borderBottom:"1px solid #e0f0ff",boxShadow:"0 1px 12px rgba(30,58,95,0.06)"}}>
        ...
        <div style={{display:"flex",gap:"2rem"}}>
          {["Lessons","Events"].map(t => (
            <button key={t} className="btn" onClick={() => setActiveTab(t.toLowerCase() as "lessons"|"events")}
              style={{background:"none",border:"none",color:activeTab===t.toLowerCase()?"#1e3a5f":"#64748b",fontWeight:600,fontSize:"0.9rem",borderBottom:activeTab===t.toLowerCase()?"2px solid #84CC16":"2px solid transparent",paddingBottom:"2px"}}>
              {t}
            </button>
          ))}
        ...
        <div style={{display:"flex",gap:"0.8rem",alignItems:"center"}}>
          {user ? (
            ...
            <>
              <button className="btn" onClick={() => { setAuthError(""); setModal("login"); }} style={{padding:"0.5rem 1.2rem",border:"1px solid #bfdbfe",borderRadius:999,background:"transparent",color:"#1e3a5f",fontSize:"0.85rem"}}>Log In</button>
              <button className="btn" onClick={() => { setAuthError(""); setModal("signup"); }} style={{padding:"0.5rem 1.4rem",border:"none",borderRadius:999,background:"#84CC16",color:"#1e3a5f",fontWeight:700,fontSize:"0.85rem"}}>Sign Up Free</button>
            </>
          )}
        </div>
      </nav>
      ...
      <section style={{padding:"5rem 2.5rem 3rem",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"inline-block",background:"rgba(132,204,22,0.12)",color:"#4d7c0f",fontSize:"0.75rem",fontWeight:700,letterSpacing:2,textTransform:"uppercase",padding:"0.35rem 1rem",borderRadius:999,marginBottom:"1.5rem",border:"1px solid rgba(132,204,22,0.3)"}}>🏙️ Now in Melbourne</div>
        <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"clamp(3.5rem,8vw,7rem)",lineHeight:0.95,marginBottom:"1.5rem",maxWidth:700,color:"#1e3a5f"}}>
          Find Your<br/><span style={{color:"#84CC16"}}>Perfect Game</span>
        </h1>
        <p style={{fontSize:"1.05rem",color:"#64748b",maxWidth:480,lineHeight:1.7,marginBottom:"2.5rem"}}>
      ...
      <div style={{display:"flex",gap:"3rem",padding:"2rem 2.5rem",borderTop:"1px solid #dbeafe",borderBottom:"1px solid #dbeafe",flexWrap:"wrap",background:"white"}}>
        {[["240+","Lesson Providers"],["18","Sports & Activities"],["3,100+","Players in Melbourne"],["4.8 ⭐","Average Rating"]].map(([n,l]) => (
          <div key={l}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"2.2rem",color:"#1e3a5f",letterSpacing:1}}>{n}</div>
      ...
      <section style={{padding:"2.5rem 2.5rem"}}>
        <div style={{display:"flex",gap:"0.5rem",background:"white",border:"1px solid #bfdbfe",borderRadius:16,padding:"0.6rem",maxWidth:900,flexWrap:"wrap",boxShadow:"0 2px 12px rgba(30,58,95,0.06)"}}>
          <input placeholder="Search lessons, sports, activities..." value={searchText} onChange={e => setSearchText(e.target.value)}
            style={{flex:2,background:"transparent",border:"none",outline:"none",color:"#1e3a5f",fontSize:"0.95rem",padding:"0.4rem 0.8rem",minWidth:160}}/>
          ...
          <input placeholder="Postcode or suburb..." value={searchPostcode} onChange={e => setSearchPostcode(e.target.value)}
            style={{flex:1,background:"transparent",border:"none",outline:"none",color:"#1e3a5f",fontSize:"0.95rem",padding:"0.4rem 0.8rem",minWidth:140}}/>
          <div style={{width:1,background:"#dbeafe",margin:"0.2rem 0"}}/>
        ...
        {(searchPostcode || searchText) && (
          <div style={{marginTop:"0.8rem",fontSize:"0.82rem",color:"#64748b",display:"flex",alignItems:"center",gap:"0.8rem"}}>
            {searchPostcode && <span>📍 Filtering by: <strong style={{color:"#1e3a5f"}}>{searchPostcode}</strong></span>}
            {searchText && <span>🔍 Searching: <strong style={{color:"#1e3a5f"}}>{searchText}</strong></span>}
        ...
        <div style={{display:"flex",gap:"0.6rem",marginTop:"1rem",flexWrap:"wrap"}}>
          {[["all","All"],["tennis","🎾 Tennis"],["piano","🎹 Piano"],["swimming","🏊 Swimming"],["yoga","🧘 Yoga"],["guitar","🎸 Guitar"],["martial arts","🥋 Martial Arts"]].map(([val,label]) => (
            <button key={val} className="btn" onClick={() => setActiveFilter(val)}
              style={{padding:"0.4rem 1rem",borderRadius:999,fontSize:"0.8rem",fontWeight:500,border:`1px solid ${activeFilter===val?"#84CC16":"#bfdbfe"}`,color:activeFilter===val?"#4d7c0f":"#64748b",background:activeFilter===val?"rgba(132,204,22,0.1)":"white"}}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div style={{display:"flex",padding:"0 2.5rem",borderBottom:"1px solid #dbeafe",marginBottom:"2rem",background:"white"}}>
        {[["lessons","📚 Lessons"],["events","🏃 Community Events"]].map(([val,label]) => (
          <button key={val} className="btn" onClick={() => setActiveTab(val as "lessons"|"events")}
            style={{padding:"0.9rem 1.8rem",fontSize:"0.9rem",fontWeight:600,color:activeTab===val?"#1e3a5f":"#64748b",background:"none",border:"none",borderBottom:`2px solid ${activeTab===val?"#84CC16":"transparent"}`}}>
            {label}
          </button>
        ))}
      </div>
      ...
      {activeTab === "lessons" && (
        <section style={{padding:"0 2.5rem 3rem"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.5rem"}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.8rem",letterSpacing:1,color:"#1e3a5f"}}>{filtered.length > 0 ? `${filtered.length} Lesson${filtered.length===1?"":"s"} Found` : "No Lessons Found"}</div>
              ...
              {filtered.map((l,i) => (
                <div key={l.id} className="lesson-card" onClick={() => router.push(`/lessons/${l.id}`)}
                  ...
                  <div style={{height:160,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"4rem",position:"relative",background:"#1e3a5f"}}>
                    {l.emoji}
                    {(l.featured || l.badge) && (
                      <div style={{position:"absolute",top:"0.8rem",right:"0.8rem",background:l.featured?"#F97316":"#84CC16",color:l.featured?"white":"#1e3a5f",fontSize:"0.7rem",fontWeight:800,padding:"0.25rem 0.6rem",borderRadius:999}}>
      ...
      {activeTab === "events" && (
        <section style={{padding:"0 2.5rem 3rem"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.5rem"}}>
            ...
            <button className="btn" onClick={() => user ? setModal("event") : setModal("login")} style={{borderRadius:999,fontSize:"0.85rem",padding:"0.5rem 1.2rem",background:"#84CC16",color:"#1e3a5f",border:"none",fontWeight:700}}>+ Create Event</button>
          </div>
          {filteredEvents.length === 0 ? (
            <div style={{textAlign:"center",padding:"3rem",color:"#64748b"}}>
              <div style={{fontSize:"3rem",marginBottom:"1rem"}}>🏃</div>
              <div style={{fontSize:"1rem",marginBottom:"0.5rem"}}>No events found for that postcode</div>
              <button className="btn" onClick={() => setSearchPostcode("")} style={{marginTop:"1rem",background:"rgba(132,204,22,0.1)",border:"1px solid rgba(132,204,22,0.3)",color:"#4d7c0f",borderRadius:999,padding:"0.5rem 1.2rem",fontSize:"0.85rem"}}>Clear Search</button>
            </div>
          ) : (
              ...
              {filteredEvents.map((ev,i) => {
                ...
                return (
                  <div key={ev.id} className="event-card" onClick={() => router.push(`/events/${ev.id}`)}
                    style={{background:"white",borderRadius:16,padding:"1.4rem",border:"1px solid #dbeafe",cursor:"pointer",position:"relative",overflow:"hidden",animation:`fadeInUp 0.4s ease ${i*0.05}s both`,transition:"all 0.25s",borderTop:`3px solid ${c}`,boxShadow:"0 2px 8px rgba(30,58,95,0.06)"}}>
                    ...
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"1rem"}}>
                      ...
                      <span style={{background:`${c}15`,color:c,border:`1px solid ${c}40`,fontSize:"0.75rem",fontWeight:700,padding:"0.3rem 0.7rem",borderRadius:999}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:"0.4rem",marginBottom:"1.2rem"}}>
                      {[["📅",ev.date],["📍",`${ev.location} (${ev.postcode})`],["👥",`${ev.joined} joined · looking for ${spotsLeft} more`],["💰",ev.cost]].map(([icon,text]) => (
                        <div key={text} style={{display:"flex",alignItems:"center",gap:"0.6rem",fontSize:"0.82rem",color:"#64748b"}}>
      ...
      <div style={{margin:"0 2.5rem 3rem",background:"#1e3a5f",borderRadius:20,padding:"2.5rem",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"2rem",flexWrap:"wrap"}}>
        <div>
          ...
          <p style={{color:"#93c5fd",fontSize:"0.9rem",maxWidth:400,lineHeight:1.6}}>List your lessons on PlayUp and reach thousands of learners across Melbourne.</p>
          <div style={{display:"flex",gap:"1.5rem",marginTop:"1.2rem",flexWrap:"wrap"}}>
            {["Free listing to start","Attendance tracking","Direct messaging","Featured placement"].map(p => (
              <div key={p} style={{display:"flex",alignItems:"center",gap:"0.5rem",fontSize:"0.82rem",color:"#93c5fd"}}>
        ...
        <button className="btn" onClick={() => user ? setModal("provider") : setModal("signup")} style={{padding:"0.85rem 2rem",fontSize:"1rem",borderRadius:999,fontWeight:700,background:"#84CC16",color:"#1e3a5f",border:"none",whiteSpace:"nowrap"}}>List My Lessons →</button>
      </div>
      ...
      <footer style={{background:"#1e3a5f",borderTop:"1px solid rgba(255,255,255,0.1)",padding:"2rem 2.5rem",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"1rem"}}>
        ...
        <div style={{display:"flex",gap:"1.5rem",flexWrap:"wrap"}}>
          {["About","For Providers","Help","Privacy","Terms"].map(l => (
            <a key={l} href="#" style={{fontSize:"0.82rem",color:"#93c5fd",textDecoration:"none"}}>{l}</a>
          ))}
        </div>
        <div style={{fontSize:"0.75rem",color:"#60a5fa"}}>© 2026 PlayUp · Melbourne, VIC</div>
      </footer>
        ...
        <div onClick={(e) => { if(e.target===e.currentTarget) setModal(null); }}
          ...
          <div style={{background:"white",border:"1px solid #bfdbfe",borderRadius:20,padding:"2rem",width:"90%",maxWidth:480,position:"relative",animation:"slideUp 0.3s ease",boxShadow:"0 20px 60px rgba(30,58,95,0.15)"}}>
            ...
            {modal === "login" && <>
              ...
              <button className="btn" onClick={handleLogin} disabled={authLoading} style={{width:"100%",padding:"0.85rem",background:"#1e3a5f",color:"white",border:"none",borderRadius:10,fontWeight:700,fontSize:"1rem",marginTop:"0.5rem",opacity:authLoading?0.7:1}}>{authLoading?"Logging in...":"Log In"}</button>
              <p style={{textAlign:"center",marginTop:"1rem",fontSize:"0.85rem",color:"#64748b"}}>Don't have an account? <span style={{color:"#84CC16",cursor:"pointer",fontWeight:700}} onClick={() => { setAuthError(""); setModal("signup"); }}>Sign up free</span></p>
            </>}
            ...
            {modal === "signup" && <>
              ...
              <button className="btn" onClick={handleSignup} disabled={authLoading} style={{width:"100%",padding:"0.85rem",background:"#84CC16",color:"#1e3a5f",border:"none",borderRadius:10,fontWeight:700,fontSize:"1rem",marginTop:"0.5rem",opacity:authLoading?0.7:1}}>{authLoading?"Creating account...":"Create Free Account"}</button>
              <p style={{textAlign:"center",marginTop:"1rem",fontSize:"0.85rem",color:"#64748b"}}>Already have an account? <span style={{color:"#1e3a5f",cursor:"pointer",fontWeight:700}} onClick={() => { setAuthError(""); setModal("login"); }}>Log in</span></p>
            </>}
            ...
            {modal === "provider" && <>
              ...
              <p style={{color:"#64748b",fontSize:"0.88rem",marginBottom:"1.5rem"}}>Get discovered by students in Melbourne</p>
              ...
              <button className="btn" onClick={() => { setModal(null); showToast("Your listing is under review!"); }} style={{width:"100%",padding:"0.85rem",background:"#1e3a5f",color:"white",border:"none",borderRadius:10,fontWeight:700,fontSize:"1rem",marginTop:"0.5rem"}}>Submit My Listing</button>
            </>}
