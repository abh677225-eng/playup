  ...
  const handleLogin = async () => {
    ...
    if (error) {
      if (error.message.includes("Invalid login credentials")) {
        setAuthError("Incorrect email or password. Try again or use Forgot Password.");
        ...
        <div style={{display:"flex",gap:"2rem"}}>
          {["Lessons","Events"].map(t => (
            <button key={t} className="btn" onClick={() => setActiveTab(t.toLowerCase() as "lessons"|"events")}
              style={{background:"none",border:"none",color:activeTab===t.toLowerCase()?"#1e3a5f":"#64748b",fontWeight:600,fontSize:"0.9rem",borderBottom:activeTab===t.toLowerCase()?"2px solid #84CC16":"2px solid transparent",paddingBottom:"2px"}}>{t}</button>
          ))}
      ...
      <section style={{padding:"2.5rem 2.5rem"}}>
        <div style={{display:"flex",gap:"0.5rem",background:"white",border:"1px solid #bfdbfe",borderRadius:16,padding:"0.6rem",maxWidth:900,flexWrap:"wrap",boxShadow:"0 2px 12px rgba(30,58,95,0.06)"}}>
          ...
          <select style={{background:"transparent",border:"none",outline:"none",color:"#64748b",fontSize:"0.9rem",padding:"0.4rem 0.8rem",cursor:"pointer"}} onChange={e=>setSearchPostcode(e.target.value)}>
            ...
            {[["Richmond","3121"],["Fitzroy","3065"],["St Kilda","3182"],["South Yarra","3141"],["Carlton","3053"],["Hawthorn","3122"],["Toorak","3142"],["Docklands","3008"]].map(([s,p]) => <option key={p} value={p}>{s} {p}</option>)}
          </select>
        ...
        <section style={{padding:"0 2.5rem 3rem"}}>
          ...
          {filtered.length===0 ? (
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:"1.2rem"}}>
              {filtered.map((l,i) => (
                <div key={l.id} className="lesson-card" onClick={()=>router.push(`/lessons/${l.id}`)} style={{background:"white",borderRadius:16,overflow:"hidden",border:"1px solid #dbeafe",cursor:"pointer",animation:`fadeInUp 0.4s ease ${i*0.05}s both`,transition:"all 0.25s",boxShadow:"0 2px 8px rgba(30,58,95,0.06)"}}>
                  <div style={{height:160,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"4rem",position:"relative",background:"#1e3a5f"}}>
                    {l.emoji}
                    {(l.featured||l.badge)&&<div style={{position:"absolute",top:"0.8rem",right:"0.8rem",background:l.featured?"#F97316":"#84CC16",color:l.featured?"white":"#1e3a5f",fontSize:"0.7rem",fontWeight:800,padding:"0.25rem 0.6rem",borderRadius:999}}>{l.featured?"Featured":l.badge}</div>}
                  </div>
          ...
          {filteredEvents.length===0 ? (
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:"1.2rem"}}>
              {filteredEvents.map((ev,i) => {
                ...
                return (
                  <div key={ev.id} className="event-card" onClick={()=>router.push(`/events/${ev.id}`)} style={{background:"white",borderRadius:16,padding:"1.4rem",border:"1px solid #dbeafe",cursor:"pointer",position:"relative",overflow:"hidden",animation:`fadeInUp 0.4s ease ${i*0.05}s both`,transition:"all 0.25s",borderTop:`3px solid ${c}`,boxShadow:"0 2px 8px rgba(30,58,95,0.06)"}}>
          ...
          <div style={{background:"white",border:"1px solid #bfdbfe",borderRadius:20,padding:"2rem",width:"90%",maxWidth:480,position:"relative",animation:"slideUp 0.3s ease",boxShadow:"0 20px 60px rgba(30,58,95,0.15)"}}>
            ...
            {modal==="login" && loginView==="login" && <>
              ...
              <div style={{textAlign:"right",marginBottom:"1.2rem"}}>
                <button className="btn" onClick={()=>{setAuthError("");setAuthSuccess("");setForgotEmail(email);setLoginView("forgot");}} style={{background:"none",border:"none",color:"#84CC16",fontSize:"0.82rem",fontWeight:600,padding:0,cursor:"pointer"}}>Forgot password?</button>
              </div>
            ...
            {modal==="provider" && <>
              ...
              <p style={{color:"#64748b",fontSize:"0.88rem",marginBottom:"1.5rem"}}>Ready to reach thousands of students in Melbourne?</p>
              <div style={{background:"#f8faff",borderRadius:12,padding:"1.2rem",border:"1px solid #dbeafe",marginBottom:"1.5rem"}}>
                {["Free listing to get started","Full pricing & booking control","Attendance tracking tools","Direct messaging with students"].map(p=>(
